...
<head>
<meta charset="UTF-8">
<title>Moon Runner - Full Game</title>
<style>
body { margin:0; overflow:hidden; background:#111; font-family:sans-serif; }
canvas { display:block; touch-action:none; }

#hud { position:absolute; top:50px; left:20px; color:white; font-size:20px; z-index:10; }
#topAd { position:absolute; top:0; left:0; width:100%; height:50px; background:#222; color:#fff; display:flex; align-items:center; justify-content:center; z-index:10; font-size:20px; }
#startOverlay, #gameOverOverlay, #videoAdOverlay {
  position:absolute; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.9); display:flex; flex-direction:column; align-items:center; justify-content:center; color:white; z-index:20;
}
#startOverlay button, #gameOverOverlay button, #videoAdOverlay button {
  padding:15px 50px; font-size:1.5em; margin-top:20px; border:none; border-radius:50px; cursor:pointer;
  background: linear-gradient(90deg,#ffdd00,#00ccff); color:black; transition: transform 0.2s, box-shadow 0.2s;
}
#startOverlay button:hover, #gameOverOverlay button:hover, #videoAdOverlay button:hover {
  transform: scale(1.1); box-shadow: 0 10px 25px rgba(0,0,0,0.7);
}
video { width:80%; max-width:600px; border-radius:10px; }
</style>
</head>
<body>
<div id="topAd">Top Banner Ad</div>
<div id="hud">Score: <span id="score">0</span></div>

<!-- Start Overlay -->
<div id="startOverlay">
  <h1>Moon Runner</h1>
  <p>Prepare to run and jump on the Moon!</p>
...
let starPositions=new Float32Array(35000*3);
...
// ===== Moon Surface =====
let moonGeo=new THREE.PlaneGeometry(50,2000,20,20);
...
let mountains=[];
for(let i=0;i<10;i++){
  let geo=new THREE.ConeGeometry(20+Math.random()*10,40+Math.random()*20,6);
  let mat=new THREE.MeshStandardMaterial({color:0x444444});
  let m=new THREE.Mesh(geo,mat);
  m.position.set([-25,25][Math.floor(Math.random()*2)]+Math.random()*10,20,-i*150);
  scene.add(m);
  mountains.push(m);
}
...
let trees=[];
for(let i=0;i<40;i++){
  let trunk=new THREE.Mesh(new THREE.CylinderGeometry(0.3,0.5,2,6),new THREE.MeshStandardMaterial({color:0x8B4513}));
  let leaves=new THREE.Mesh(new THREE.ConeGeometry(2,4,6),new THREE.MeshStandardMaterial({color:0x228B22}));
  let tree=new THREE.Group();
  trunk.position.y=1;
  leaves.position.y=3;
  tree.add(trunk); tree.add(leaves);
  tree.position.set([-18,18][Math.floor(Math.random()*2)],0,-i*50);
  scene.add(tree);
  trees.push(tree);
}
...
function addLamp(x,z){
  ...
  let ring=new THREE.Mesh(new THREE.TorusGeometry(0.6,0.1,16,32),new THREE.MeshStandardMaterial({color:0xffee88,emissive:0xffdd88,emissiveIntensity:2}));
  ring.rotation.x=Math.PI/2; ring.position.y=6; lamp.add(ring);
  let glow=new THREE.PointLight(0xffee88,2.5,25,2); glow.position.y=6; lamp.add(glow);
  lamp.position.set(x,0,z);
  scene.add(lamp);
  lamps.push(lamp);
}
for(let i=0;i<25;i++){ addLamp(-7,-i*40); addLamp(7,-i*40); }
...
let obstacles=[];
for(let i=0;i<10;i++){
  let o=new THREE.Mesh(new THREE.DodecahedronGeometry(Math.random()*1+1), new THREE.MeshStandardMaterial({color:0x666666}));
  o.position.set([-4,0,4][Math.floor(Math.random()*3)],1,-500-Math.random()*800);
  scene.add(o); obstacles.push(o);
}

// ===== Rolling Rocks =====
let rollingRocks=[]; let maxRocks=6;
for(let i=0;i<maxRocks;i++){ let rock=new THREE.Mesh(new THREE.SphereGeometry(1+Math.random()*0.5,12,12), new THREE.MeshStandardMaterial({color:0x888888})); rock.visible=false; scene.add(rock); rollingRocks.push(rock); }
...
// ===== State =====
let lane=0, targetLane=0; let isJumping=false, jumpVel=0, gravity=-0.012;
...
document.addEventListener("touchend",e=>{
  ...
  if(Math.abs(dx)>Math.abs(dy)){
    ...
    if(dy<-50 && !isJumping){ isJumping=true; jumpVel=0.3; }
  }
});
...
// ===== Rolling Rocks Spawn =====
let lastRockTime=0;
function spawnRollingRock(){ if(Date.now()-lastRockTime<600) return; lastRockTime=Date.now(); let trackX=[-4,0,4][Math.floor(Math.random()*3)]; for(let r of rollingRocks){ if(!r.visible){ r.position.set(trackX,1.5,player.position.z-120); r.visible=true; break; } } }
...
function animate(){
  ...
  let leanAngle=(targetLane - player.position.x/4)*0.2;
  torso.rotation.z=leanAngle;
  ...
  mountains.forEach(m=>{ m.position.z+=speed*0.4; if(m.position.z>50) m.position.z=-2000+Math.random()*500; });
  ...
  obstacles.forEach(o=>{
    ...
    if(o.position.z>10){ o.position.z=-800-Math.random()*800; o.position.x=[-4,0,4][Math.floor(Math.random()*3)]; }
    let dx=Math.abs(player.position.x-o.position.x), dz=Math.abs(player.position.z-o.position.z);
    if(dx<1.2 && dz<1.2 && player.position.y<1.6) endGame();
  });

  spawnRollingRock();
  for(let r of rollingRocks){
    if(r.visible){ r.position.z+=speed+0.7; r.rotation.y+=0.05; if(r.position.z>player.position.z+15) r.visible=false; let dx=Math.abs(player.position.x-r.position.x), dz=Math.abs(player.position.z-r.position.z); if(dx<1.5 && dz<1.5) endGame(); }
  }

  ufoObstacles.forEach(ufo=>{
    ufo.position.z+=speed; ufo.rotation.y+=0.02; ufo.position.y=1.5+Math.sin(Date.now()*0.002)*0.2;
...
document.getElementById("startBtn").onclick=()=>{
  gameStarted=true; document.getElementById("startOverlay").style.display="none"; animate();
};
...
document.getElementById("watchAdBtn").onclick=()=>{
  document.getElementById("videoAdOverlay").style.display="flex";
...
// ===== Continue After Ad =====
document.getElementById("continueAfterAdBtn").onclick=()=>{
  document.getElementById("videoAdOverlay").style.display="none";
  gameOver=false;
  player.position.set(playerDeathPosition.x,playerDeathPosition.y,playerDeathPosition.z);
  animate();
};
